...
#  if defined(HCPP_LEGACY_HANDLE_API)
  ...
  // incompatibilities with legacy code (especially HMatrix to HTuple). Instead
...
#  endif
  ...
  static HTuple TupleConstant(const HTuple& Name);

  // Check a tuple whether it represents NaNs (Not-a-number).
  HTuple TupleIsNanElem() const;

  // Repeat a tuple.
  HTuple TupleRepeat(const HTuple& Num) const;

  // Repeat the elements of a tuple.
  HTuple TupleRepeatElem(const HTuple& Num) const;
