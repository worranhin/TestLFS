...
LIntExport void SampleObjectModel3d(const HTuple& ObjectModel3D, const HTuple& Method, const HTuple& SamplingParam, const HTuple& GenParamName, const HTuple& GenParamValue, HTuple* SampledObjectModel3D);
...
LIntExport void GrayHistoRange(const HObject& Region, const HObject& Image, const HTuple& Min, const HTuple& Max, const HTuple& NumBins, HTuple* Histo, HTuple* BinSize);
...
LIntExport void GrayHistoAbs(const HObject& Region, const HObject& Image, const HTuple& Quantization, HTuple* AbsoluteHisto);

// Calculate the gray value distribution.
LIntExport void GrayHisto(const HObject& Region, const HObject& Image, HTuple* AbsoluteHisto, HTuple* RelativeHisto);
...
LIntExport void CompareMemoryBlock(const HTuple& MemoryBlocks1, const HTuple& MemoryBlocks2, HTuple* IsEqual);

// Create a depth to space layer.
LIntExport void CreateDlLayerDepthToSpace(const HTuple& DLLayerInput, const HTuple& LayerName, const HTuple& BlockSize, const HTuple& Mode, const HTuple& GenParamName, const HTuple& GenParamValue, HTuple* DLLayerDepthToSpace);

// Create an identity layer.
LIntExport void CreateDlLayerIdentity(const HTuple& DLLayerInput, const HTuple& LayerName, const HTuple& GenParamName, const HTuple& GenParamValue, HTuple* DLLayerIdentity);

// Create a memory block from an external pointer.
LIntExport void CreateMemoryBlockExtern(const HTuple& Pointer, const HTuple& Size, const HTuple& FreeFunction, HTuple* MemoryBlockHandle);

// Create a memory block from an external pointer by copying.
LIntExport void CreateMemoryBlockExternCopy(const HTuple& Pointer, const HTuple& Size, HTuple* MemoryBlockHandle);

// Decrypt an encrypted item.
LIntExport void DecryptSerializedItem(const HTuple& EncryptedItemHandle, const HTuple& DecryptionParam, HTuple* SerializedItemHandle);

// Encrypt a serialized item.
LIntExport void EncryptSerializedItem(const HTuple& SerializedItemHandle, const HTuple& EncryptionParam, HTuple* EncryptedItemHandle);

// Return the data pointer and size of a memory block.
LIntExport void GetMemoryBlockPtr(const HTuple& MemoryBlockHandle, HTuple* Pointer, HTuple* Size);

// Write an image to a memory block in various graphic formats.
LIntExport void ImageToMemoryBlock(const HObject& Image, const HTuple& Format, const HTuple& FillColor, HTuple* MemoryBlockHandle);

// Read an image from a memory block with different file formats.
LIntExport void MemoryBlockToImage(HObject* Image, const HTuple& MemoryBlockHandle);

// Read an encrypted item from a file.
LIntExport void ReadEncryptedItem(const HTuple& FileName, HTuple* EncryptedItemHandle);

// Read a memory block from a file.
LIntExport void ReadMemoryBlock(const HTuple& FileName, HTuple* MemoryBlockHandle);

// Generate a tuple with the value of a HDevelop language constant.
LIntExport void TupleConstant(const HTuple& Name, HTuple* Value);

// Check a tuple whether it represents NaNs (Not-a-number).
LIntExport void TupleIsNanElem(const HTuple& T, HTuple* IsNaN);

// Write an encrypted item to a file.
LIntExport void WriteEncryptedItem(const HTuple& EncryptedItemHandle, const HTuple& FileName);

// Write a memory block to a file.
LIntExport void WriteMemoryBlock(const HTuple& MemoryBlockHandle, const HTuple& FileName);

// Add a border to an image.
LIntExport void AddImageBorder(const HObject& Image, HObject* ImageBorder, const HTuple& Size, const HTuple& Value);

// Apply a Deep Counting model on a set of images for inference. 
LIntExport void ApplyDeepCountingModel(const HObject& Image, const HTuple& DeepCountingHandle, HTuple* Count, HTuple* DeepCountingResult);

// Create a Deep Counting model for counting objects.
LIntExport void CreateDeepCountingModel(const HTuple& GenParamName, const HTuple& GenParamValue, HTuple* DeepCountingHandle);

// Create a MatMul layer.
LIntExport void CreateDlLayerMatmul(const HTuple& DLLayerA, const HTuple& DLLayerB, const HTuple& LayerName, const HTuple& GenParamName, const HTuple& GenParamValue, HTuple* DLLayerMatMul);

// Return the parameters of a Deep Counting model.
LIntExport void GetDeepCountingModelParam(const HTuple& DeepCountingHandle, const HTuple& GenParamName, HTuple* GenParamValue);

// Set templates of a Deep Counting model.
LIntExport void PrepareDeepCountingModel(const HObject& Templates, const HTuple& DeepCountingHandle);

// Read a Deep Counting model from a file.
LIntExport void ReadDeepCountingModel(const HTuple& FileName, HTuple* DeepCountingHandle);

// Set selected parameters of the Deep Counting model.
LIntExport void SetDeepCountingModelParam(const HTuple& DeepCountingHandle, const HTuple& GenParamName, const HTuple& GenParamValue);

// Write a Deep Counting model in a file.
LIntExport void WriteDeepCountingModel(const HTuple& DeepCountingHandle, const HTuple& FileName);

// Create a reduce layer.
LIntExport void CreateDlLayerReduce(const HTuple& DLLayerInput, const HTuple& LayerName, const HTuple& Operation, const HTuple& Axes, const HTuple& GenParamName, const HTuple& GenParamValue, HTuple* DLLayerReduce);

// Reconstruct a surface from a decoded structured light setup.
LIntExport void ReconstructSurfaceStructuredLight(const HTuple& StructuredLightModel, HTuple* ObjectModel3D);

// Intersect a contour with a region.
LIntExport void IntersectionRegionContourXld(const HObject& Region, const HObject& Contour, HObject* ContourIntersection, const HTuple& Mode);

// Repeat a tuple.
LIntExport void TupleRepeat(const HTuple& Tuple, const HTuple& Num, HTuple* Result);

// Repeat the elements of a tuple.
LIntExport void TupleRepeatElem(const HTuple& Tuple, const HTuple& Num, HTuple* Result);

// Replace all occurrences of a substring within a string.
LIntExport void TupleStrReplace(const HTuple& String, const HTuple& Before, const HTuple& After, HTuple* Replaced);

// Find the pose of objects using Deep 3D Matching.
LIntExport void ApplyDeepMatching3d(const HObject& Images, const HTuple& Deep3DMatchingModel, HTuple* DeepMatchingResults);

// Extend a deep learning model for Out-of-Distribution Detection.
LIntExport void FitDlOutOfDistribution(const HTuple& DLModelHandle, const HTuple& DLDataset, const HTuple& GenParam);

// Read a parameter from a Deep 3D Matching model.
LIntExport void GetDeepMatching3dParam(const HTuple& Deep3DMatchingModel, const HTuple& GenParamName, HTuple* GenParamValue);

// Read a Deep 3D Matching model from a file.
LIntExport void ReadDeepMatching3d(const HTuple& FileName, HTuple* Deep3DMatchingModel);

// Set a parameter of a Deep 3D Matching model.
LIntExport void SetDeepMatching3dParam(const HTuple& Deep3DMatchingModel, const HTuple& GenParamName, const HTuple& GenParamValue);

// Test if points are contained in regions.
LIntExport void TestRegionPoints(const HObject& Regions, const HTuple& Row, const HTuple& Column, HTuple* IsInside);

// Calculate the distance between strings.
LIntExport void TupleStrDistance(const HTuple& String1, const HTuple& String2, const HTuple& Mode, HTuple* Distance);

// Write a Deep 3D Matching model in a file.
LIntExport void WriteDeepMatching3d(const HTuple& Deep3DMatchingModel, const HTuple& FileName);
