...
public:
  ...
  HSmartPtr& operator=(HSmartPtr const& ptr);
  HSmartPtr& operator=(T* ptr);
...
template<class T> void HSmartPtr<T>::deref()
{
  if (mPtr)
  {
    /* It is necessary to invalidate the smart pointer before releasing the
     * referred object for avoiding that within the update handlers of the
