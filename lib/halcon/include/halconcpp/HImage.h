...
public:
  ...
  HImage& operator=(const HImage&) = default;
#endif
...
HTuple Height() const;
  ...
  static HRegion GetFoundComponentModel(const HComponentModel& ComponentModelID, const HTuple& ModelStart, const HTuple& ModelEnd, const HTuple& RowComp, const HTuple& ColumnComp, const HTuple& AngleComp, const HTuple& ScoreComp, const HTuple& ModelComp, Hlong ModelMatch, const HString& MarkOrientation, HTuple* RowCompInst, HTuple* ColumnCompInst, HTuple* AngleCompInst, HTuple* ScoreCompInst);

  // Return the components of a found instance of a component model.
  static HRegion GetFoundComponentModel(const HComponentModel& ComponentModelID, Hlong ModelStart, Hlong ModelEnd, double RowComp, double ColumnComp, double AngleComp, double ScoreComp, Hlong ModelComp, Hlong ModelMatch, const HString& MarkOrientation, double* RowCompInst, double* ColumnCompInst, double* AngleCompInst, double* ScoreCompInst);
  ...
  HTuple GrayHistoRange(const HRegion& Region, const HTuple& Min, const HTuple& Max, Hlong NumBins, double* BinSize) const;

  // Calculate the gray value distribution of a single channel image within a certain gray value range.
  Hlong GrayHistoRange(const HRegion& Region, double Min, double Max, Hlong NumBins, double* BinSize) const;
  ...
  HTuple GrayHistoAbs(const HRegion& Region, const HTuple& Quantization) const;

  // Calculate the gray value distribution.
  HTuple GrayHistoAbs(const HRegion& Region, double Quantization) const;

  // Calculate the gray value distribution.
  HTuple GrayHisto(const HRegion& Region, HTuple* RelativeHisto) const;
  ...
  HMemoryBlock ImageToMemoryBlock(const HString& Format, const HTuple& FillColor) const;

  // Write an image to a memory block in various graphic formats.
  HMemoryBlock ImageToMemoryBlock(const HString& Format, Hlong FillColor) const;

  // Write an image to a memory block in various graphic formats.
  HMemoryBlock ImageToMemoryBlock(const char* Format, Hlong FillColor) const;

#ifdef _WIN32
  // Write an image to a memory block in various graphic formats.
  HMemoryBlock ImageToMemoryBlock(const wchar_t* Format, Hlong FillColor) const;
#endif

  // Read an image from a memory block with different file formats.
  void MemoryBlockToImage(const HMemoryBlock& MemoryBlockHandle);

  // Add a border to an image.
  HImage AddImageBorder(const HTuple& Size, const HTuple& Value) const;

  // Add a border to an image.
  HImage AddImageBorder(Hlong Size, Hlong Value) const;
