...
#    define HC_DEPRECATED(MSG) [[deprecated(MSG)]]
...
#    define HC_DEPRECATED(MSG) __declspec(deprecated("is deprecated: " MSG))
#  endif
#endif
...
#define DIST(PIX, PIX2, ABSDIF, IMAGE_IN_KIND)                                \
  ...
  case CYCLIC_IMAGE:                                                          \
  {                                                                           \
    double dmax  = (double)UCHAR_MAX + 1.;                                    \
    double dmaxh = (double)(int)(dmax / 2.);                                  \
    ABSDIF       = ABS((PIX) - (PIX2));                                       \
    if ((ABSDIF) > dmaxh)                                                     \
      ABSDIF = dmax - (ABSDIF);                                               \
  }                                                                           \
...
#    define XHCkP(Proc)                                                       \
      ...
      {                                                                       \
        ...
        Herror const _H_ERR = (Proc);                                         \
        if (HIsError(_H_ERR))                                                 \
        {                                                                     \
          Herror const _H_ERR_P =                                             \
...
#  define XHCkPCloseFile(Proc, File)                                          \
    ...
    {                                                                         \
      if (IOGetSpyProcCallSwitch())                                           \
        IOSpyProcCall(#Proc, __LINE__, __FILE__);                             \
      Herror const ERR = (Proc);                                              \
      if (HIsError(ERR))                                                      \
      {                                                                       \
        fclose(File);                                                         \
        Herror H_ERR_P = HProcessErr(#Proc, ERR, __LINE__, __FILE__);         \
        if (HIsError(H_ERR_P))                                                \
          return H_ERR_P;                                                     \
        return ERR;                                                           \
      }                                                                       \
...
#  define HCkRL(PROC_HANDLE, RL, PROC, IN_OUT)                                \
    ...
    {                                                                         \
      ...
      {                                                                       \
        Herror H_ERR_RL = HRLTest(PROC_HANDLE, RL, false);                    \
...
#define HGetCParNum(PROC_HANDLE, Par, Num)                                    \
  {                                                                           \
    Herror const ERR = HPGetCParNum(PROC_HANDLE, Par, Num);                   \
    if (HIsError(ERR))                                                        \
      return ERR;                                                             \
  }
...
#  define HGetPElemL(PROC_HANDLE, PAR, CONVERT, ELEM, N)                      \
    do                                                                        \
    {                                                                         \
      Herror _ERR = HPGetPElemL(PROC_HANDLE, PAR, CONVERT, ELEM, N);          \
      if (HIsError(_ERR))                                                     \
        return _ERR;                                                          \
...
#  define HGetPElemD(PROC_HANDLE, PAR, CONVERT, ELEM, N)                      \
    do                                                                        \
    {                                                                         \
      Herror _ERR = HPGetPElemD(PROC_HANDLE, PAR, CONVERT, ELEM, N);          \
      if (HIsError(_ERR))                                                     \
        return _ERR;                                                          \
...
#  define HGetPElemS(PROC_HANDLE, PAR, CONVERT, ELEM, N)                      \
    do                                                                        \
    {                                                                         \
      Herror _ERR = HPGetPElemS(PROC_HANDLE, PAR, CONVERT, ELEM, N);          \
      if (HIsError(_ERR))                                                     \
        return _ERR;                                                          \
...
#  define HGetPElem(PROC_HANDLE, PAR, ELEM, N, TYPE)                          \
    ...
    {                                                                         \
      ...
      Herror _ERR = IOSpyElem(PROC_HANDLE, PAR, *(void**)(ELEM),              \
                              *(INT4_8*)(N), *(int*)(TYPE), true);            \
      if (HIsError(_ERR))                                                     \
        return _ERR;                                                          \
    } while (0)
#else
...
#  define HGetElemL(PROC_HANDLE, PAR, CONVERT, MEM_TYPE, ELEM, N)             \
    do                                                                        \
    {                                                                         \
      Herror _ERR = HPGetElemL(PROC_HANDLE, PAR, CONVERT, MEM_TYPE, ELEM, N); \
      if (HIsError(_ERR))                                                     \
        return _ERR;                                                          \
      _ERR = IOSpyElem(PROC_HANDLE, PAR, *(INT4_8**)(ELEM), *(INT4_8*)(N),    \
                       LONG_PAR, true);                                       \
...
#  define HGetElemD(PROC_HANDLE, PAR, CONVERT, MEM_TYPE, ELEM, N)             \
    do                                                                        \
    {                                                                         \
      Herror _ERR = HPGetElemD(PROC_HANDLE, PAR, CONVERT, MEM_TYPE, ELEM, N); \
      if (HIsError(_ERR))                                                     \
        return _ERR;                                                          \
      _ERR = IOSpyElem(PROC_HANDLE, PAR, *(double**)(ELEM), *(INT4_8*)(N),    \
                       DOUBLE_PAR, true);                                     \
...
#  define HGetElemS(PROC_HANDLE, PAR, CONVERT, MEM_TYPE, ELEM, N)             \
    do                                                                        \
    {                                                                         \
      Herror _ERR = HPGetElemS(PROC_HANDLE, PAR, CONVERT, MEM_TYPE, ELEM, N); \
      if (HIsError(_ERR))                                                     \
        return _ERR;                                                          \
      _ERR = IOSpyElem(PROC_HANDLE, PAR, *(char***)(ELEM), *(INT4_8*)(N),     \
                       STRING_PAR, true);                                     \
...
#  define HCopyElemL(PROC_HANDLE, PAR, CONVERT, ELEM, N)                      \
    do                                                                        \
    {                                                                         \
      Herror _ERR = HPCopyElemL(PROC_HANDLE, PAR, CONVERT, ELEM, N);          \
      if (HIsError(_ERR))                                                     \
        return _ERR;                                                          \
      _ERR =                                                                  \
...
#  define HCopyElemD(PROC_HANDLE, PAR, CONVERT, ELEM, N)                      \
    do                                                                        \
    {                                                                         \
      Herror _ERR = HPCopyElemD(PROC_HANDLE, PAR, CONVERT, ELEM, N);          \
      if (HIsError(_ERR))                                                     \
        return _ERR;                                                          \
      _ERR =                                                                  \
...
#  define HGetPParMM(PROC_HANDLE, PAR, MIN, MAX, VAL, NUM)                    \
    {                                                                         \
      Herror const ERR = HPGetPPar(PROC_HANDLE, PAR, VAL, NUM);               \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
      if (*NUM < MIN || *NUM > MAX)                                           \
        return (H_ERR_WIPN1 - 1) + (PAR);                                     \
...
#  define HGetPParT(PROC_HANDLE, PAR, TYPE, VAL, NUM)                         \
    {                                                                         \
      ...
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
      for (INT4_8 _IDX = 0; _IDX < *(NUM); _IDX++)                            \
...
#  define HGetPParTN(PROC_HANDLE, PAR, TYPE, NUM, VAL)                        \
    {                                                                         \
      ...
      if ((NUM) != _NUM)                                                      \
        return (H_ERR_WIPN1 - 1) + (PAR);                                     \
      for (INT4_8 _IDX = 0; _IDX < _NUM; _IDX++)                              \
        if (((TYPE) & (*(VAL))[_IDX].type) == 0)                              \
          return (H_ERR_WIPT1 - 1) + (PAR);                                   \
      ERR = IOSpyCPar(PROC_HANDLE, PAR, *(VAL), _NUM, true);                  \
      if (HIsError(ERR))                                                      \
...
#  define HGetPParTN(PROC_HANDLE, PAR, TYPE, NUM, VAL)                        \
    {                                                                         \
      INT4_8       _NUM;                                                      \
      Herror const ERR = HPGetPPar(PROC_HANDLE, PAR, VAL, &_NUM);             \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
      if ((NUM) != _NUM)                                                      \
        return (H_ERR_WIPN1 - 1) + (PAR);                                     \
...
#  define HGetPPar(PROC_HANDLE, P, V, N)                                      \
    {                                                                         \
      Herror ERR = HPGetPPar(PROC_HANDLE, P, V, N);                           \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
...
#  define HGetPPar(PROC_HANDLE, P, V, N)                                      \
    {                                                                         \
      Herror const ERR = HPGetPPar(PROC_HANDLE, P, V, N);                     \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
    }
#endif
...
#  define HGetPar(PROC_HANDLE, Par, Kind, Val, Num)                           \
    {                                                                         \
      Herror ERR = HPGetPar(PROC_HANDLE, Par, ANY_ELEM, Kind, Val, (INT4_8)0, \
                            (INT4_8)10000000, Num);                           \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
...
#  define HGetPar(Par, Kind, Val, Num)                                        \
    {                                                                         \
      Herror const ERR = HPGetPar(Par, ANY_ELEM, Kind, Val, (INT4_8)0,        \
                                  (INT4_8)10000000, Num);                     \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
    }
...
#  define HGetSPar(PROC_HANDLE, Index, Type, Val, N)                          \
    {                                                                         \
      INT4_8 _HN;                                                             \
      Herror ERR = HPGetCPar(PROC_HANDLE, Index, Type, Val, (INT4_8)(N),      \
                             (INT4_8)(N), &_HN);                              \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
...
#  define HGetSPar(PROC_HANDLE, Index, Type, Val, N)                          \
    {                                                                         \
      INT4_8       _HN;                                                       \
      Herror const ERR = HPGetCPar(PROC_HANDLE, Index, Type, Val,             \
                                   (INT4_8)(N), (INT4_8)(N), &_HN);           \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
    }
#endif
...
#  define HGetCPar(PROC_HANDLE, Index, InpType, Val, Min, Max, ResNum)        \
    {                                                                         \
      Herror ERR = HPGetCPar(PROC_HANDLE, Index, InpType, Val, (INT4_8)(Min), \
                             (INT4_8)(Max), ResNum);                          \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
...
#  define HGetCPar(PROC_HANDLE, Index, InpType, Val, Min, Max, ResNum)        \
    {                                                                         \
      Herror const ERR = HPGetCPar(PROC_HANDLE, Index, InpType, Val,          \
                                   (INT4_8)(Min), (INT4_8)(Max), ResNum);     \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
    }
#endif
...
#  define HGetFPar(PROC_HANDLE, Index, Type, Val, Num)                        \
    {                                                                         \
      int    _HT;                                                             \
      INT4_8 _HN;                                                             \
      Herror ERR = HPGetPar(PROC_HANDLE, Index, Type, &_HT, Val,              \
                            (INT4_8)(Num), (INT4_8)(Num), &_HN);              \
      if (ERR != H_MSG_OK)                                                    \
        return ERR;                                                           \
...
#  define HGetFPar(PROC_HANDLE, Index, Type, Val, Num)                        \
    {                                                                         \
      int          _HT;                                                       \
      INT4_8       _HN;                                                       \
      Herror const ERR = HPGetPar(PROC_HANDLE, Index, Type, &_HT, Val,        \
                                  (INT4_8)(Num), (INT4_8)(Num), &_HN);        \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
    }
...
#  define HGetEPar(PROC_HANDLE, Index, InpType, ResType, Val, Min, Max, Num)  \
    {                                                                         \
      Herror ERR = HPGetPar(PROC_HANDLE, Index, InpType, ResType, Val,        \
...
#  define HGetEPar(PROC_HANDLE, Index, InpType, ResType, Val, Min, Max, Num)  \
    {                                                                         \
      Herror const ERR = HPGetPar(PROC_HANDLE, Index, InpType, ResType, Val,  \
                                  (INT4_8)(Min), (INT4_8)(Max), Num);         \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
    }
...
#  define HPutPar(PROC_HANDLE, P, K, V, N)                                    \
    {                                                                         \
      Herror ERR = IOSpyPar(PROC_HANDLE, P, K, V, (INT4_8)(N), false);        \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
...
#  define HPutPar(PROC_HANDLE, P, K, V, N)                                    \
    {                                                                         \
      Herror const ERR = HPPutPar(PROC_HANDLE, P, K, V, (INT4_8)(N));         \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
    }
#endif
...
#  define HPutCPar(PROC_HANDLE, P, V, N)                                      \
    {                                                                         \
      Herror ERR = IOSpyCPar(PROC_HANDLE, P, V, (INT4_8)N, false);            \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
...
#  define HPutCPar(PROC_HANDLE, P, V, N)                                      \
    {                                                                         \
      Herror const ERR = HPPutCPar(PROC_HANDLE, P, V, (INT4_8)(N));           \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
    }
#endif
...
#  define HPutPPar(PROC_HANDLE, P, V, N)                                      \
    {                                                                         \
      Herror const ERR = HPPutPPar(PROC_HANDLE, P, V, N);                     \
      if (HIsError(ERR))                                                      \
        return ERR;                                                           \
    }
#endif
...
#define HBAndGV(ProcHandle, Element, Value)                                   \
  HCkP(HAccessGlVar(ProcHandle, Element, GV_BAND_INFO, NULL, (double)Value,   \
                    (char*)NULL, (Hkey)NULL, 0))
/* - - - -  Old macros for accessing HALCON operator context information - - */
/* (all accesses using the old macros should be replaced by the new ones in  */
/* the long term)                                                            */
